...
from utils import read_structures_descriptions
...
def main():
    ...
    parser.add_argument("--input-cif-path", type=Path, required=True)
    parser.add_argument("--result-experiment-path", type=Path, required=True)
    args = parser.parse_args()

    all_structures = [read_structures_descriptions(args.input_cif_path / dataset_name)
    ...
    if args.strategy == "train_test":
        if args.test_size > 0:
            ...
            output_path = args.result_experiment_path.joinpath(args.experiment_name + '_validation')
            ...
            output_path = args.result_experiment_path.joinpath(args.experiment_name + '_test')
            output_path.mkdir(exist_ok=True)
        ...
        output_path = args.result_experiment_path.joinpath(args.experiment_name)
        output_path.mkdir(exist_ok=True)
